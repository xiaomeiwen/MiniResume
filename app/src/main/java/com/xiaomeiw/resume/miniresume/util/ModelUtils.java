...
public class ModelUtils {
    ...
    private static Gson gsonForDesialization = new GsonBuilder().registerTypeAdapter(Uri.class, new UriDeseriliazer()).create();
    ...
    public static <T> T read(Context context, String key, TypeToken<T> typeToken) {
        SharedPreferences sp = context.getApplicationContext().getSharedPreferences(
                PREF_NAME, Context.MODE_PRIVATE);
        try {
            return gsonForDesialization.fromJson(sp.getString(key, ""), typeToken.getType());
    ...
    private static class UriDeseriliazer implements JsonDeserializer<Uri> {
